...
| Nome | Usuário GitHub |
|---|---|
| Arthur Felipe Bach Biancolini | [Tuizones](https://github.com/Tuizones) |
| Emanuel Riceto da Silva | [emanuelriceto](https://github.com/emanuelriceto) |
| Frederico Virmond Fruet | [fredfruet](https://github.com/fredfruet) |
...
A Fase 3 foi dividida em quatro responsabilidades (uma por integrante):

| Aluno | Responsabilidade principal | Arquivos centrais |
|---|---|---|
| **Arthur F. B. Biancolini** *(Tuizones)* | Lexer + comentários (§1) e `prepararEntradaSemantica` (§3); coordenação do CLI (§7) e dos testes end-to-end | [src/lexer_fsm.py](src/lexer_fsm.py), [src/pipeline.py](src/pipeline.py), [AnalisadorSemantico.py](AnalisadorSemantico.py), [tests/test_arquivos_e2e.py](tests/test_arquivos_e2e.py) |
| **Emanuel Riceto da Silva** *(emanuelriceto)* | Tabela de símbolos (§2) e relatório `tabela_simbolos.md`; testes correspondentes | [src/semantica.py](src/semantica.py) (`TabelaSimbolos`, `construirTabelaSimbolos`, `salvarTabelaSimbolos`), [tests/test_semantica.py](tests/test_semantica.py) |
| **Frederico Virmond Fruet** *(fredfruet)* | Verificação de tipos (§4) e [regras_tipos.md](regras_tipos.md); testes de tipos | [src/semantica.py](src/semantica.py) (`verificarTipos`, `_OPS_*`), [regras_tipos.md](regras_tipos.md) |
| **Equipe (rotação)** | Árvore atribuída (§5), gerador ARMv7 tipado (§6), [gramatica_atribuida.md](gramatica_atribuida.md), arquivos de teste (§9), revisão final | [src/semantica.py](src/semantica.py) (`gerarArvoreAtribuida`), [src/armv7_generator.py](src/armv7_generator.py), `teste*.txt` |

Cada integrante manteve commits/PRs identificáveis em sua área. A
revisão final, integração do CLI e suite de testes end-to-end foi feita
em conjunto.
